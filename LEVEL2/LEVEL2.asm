...
;	Alters A,B,C,D,E,H,L
VNEG	FTYPE			;SEE WHAT KIND OF NUMBER WE HAVE
...
;	Alters A,H,L
VSIGN	FTYPE			;IS THE ARGUMENT AN INTEGER?
	JP	Z,TMERR		;BLOW UP ON STRINGS
...
ADRFAC	LD	DE,FACLO	;GET FIRST ADDRESS FOR INT, SNG
	FTYPE			;GET THE VALUE TYPE - DO WE HAVE DBL?
...
;	Alters A,B,C,D,E,H,L
FRCSNG	FTYPE			;SEE WHAT KIND OF NUMBER WE HAVE
...
;	Alters A,B,C,D,E,H,L
FRCDBL	FTYPE			;SEE WHAT KIND OF NUMBER WE HAVE
...
;	ALTERS A ONLY
;
CHKSTR	FTYPE			;SEE WHAT KIND OF VALUE WE HAVE
...
;	Greatest integer function
;	Alters A,B,C,D,E,H,L
INTFNC	FTYPE			;SEE WHAT KIND OF NUMBER WE HAVE
...
;	HERE TO CHECK FOR THE SIGN OF THE EXPONENT
	...
	CP	MINUTK		;CHECK IF THE EXPONENT IS NEGATIVE
	RET	Z		;IT IS
	CP	'-'		;THIS IS IN CASE WE ARE CALLED BY VAL
	RET	Z
	INC	D		;EXPONENT IS STILL POSITIVE, RESET FLAG
	CP	PLUSTK		;IGNORE A LEADING PLUS SIGN
	RET	Z
	CP	'+'		
	RET	Z
...
;	point flag
FINDP1	FTYPE			;SET CARRY IF WE DON'T HAVE A DOUBLE
	INC	C		;SET THE FLAG
...
FINMLT	PUSH	AF		;SAVE EXPONENT, ENTRY FROM FOUT
	FTYPE			;SEE WHAT KIND OF NUMBER WE HAVE
...
FINDIG	PUSH	DE		;SAVE EXPONENT INFORMATION
	...
	FTYPE			;SEE WHAT KIND OF A NUMBER WE HAVE
...
;	HERE TO PACK THE NEXT DIGIT OF AN INTEGER
	...
	LD	DE,3277		;SEE IF WE WILL OVERFLOW (32767/10+1)
...
FINEDG	LD	A,E		;EXPONENT DIGIT -- MULTIPLY EXPONENT BY 10
	...
	JR	NC,FINEDG1	;WE ALREADY HAVE TWO DIGITS
	RLCA			;FIRST BY 4
	RLCA
...
;	Invalid digit...
FINEDG1	LD	E,'2'		;AN EXPONENT LIKE THIS WILL SAFELY
...
LINPRT	CALL	MAKINT		;PUT THE LINE NUMBER IN THE FAC AS AN INTEGER
	...
	CALL	FOUT2		;CONVERT THE NUMBER INTO DIGITS
...
FOTSZ1	LD	(HL),C		;FILL IN THE ZERO OR THE SIGN
	...
	JR	Z,FOTSZA	;IF WE SEE A REAL ZERO, IT IS THE END OF THE NUMBER,
	...
	JR	Z,FOTSZA	; AN "E" OR A "D" SO WE CAN PRINT 0 IN
...
FOUFRV	CP	05H		;SET CC'S FOR Z80
	PUSH	HL		;SAVE THE BUFFER POINTER
...
FOFLDN	PUSH	AF		;SAVE THE EXPONENT
	FTYPE			;GET THE VALTYP TO DECIDE IF WE PRINT AN "E"
	LD	A,'"'		;[A]="D"/2
...
FOUTDN	LD	(HL),00H	;PUT A ZERO AT THE END OF THE NUMBER
...
;	HERE TO PRINT AN INTEGER IN FIXED FORMAT-FIXED POINT NOTATION
	LD	BC,0603H	;SET DECIMAL POINT COUNT TO 6 AND
				; COMMA COUNT TO 3
...
;	(It would take more than 4 bytes to check for this.)
	...
	CALL	FOUSET		;SETUP DECIMAL POINT AND COMMA COUNT
	CALL	FOUTCV		;CONVERT THE NUMBER TO DECIMAL DIGITS
	...
	JP	FOUFX1		;GO CHECK THE SIZE, ZERO SUPPRESS,
...
FFXXVS	LD	E,A		;SAVE THE EXPONENT, WE DON'T NEED WHAT IS IN E
	...
	ADD	A,E		; DIGITS
	JP	M,FFXXV1	;FOR LATER CALCULATIONS, WE WANT A ZERO IF THE
	XOR	A		; RESULT WAS NOT NEGATIVE
FFXXV1	PUSH	BC		;SAVE THE FIELD SPECS
...
;	Here to print numbers with integer digits
	...
	CALL	FOUSET		;SET UP DECIMAL POINT AND COMMA COUNT
...
FFXLV1	CALL	M,FINDIV	; OF SOME BY DIVIDING BY TEN AND ROUNDING
	...
	JP	M,FFXLV11	;TAKE INTO ACCOUNT DIGITS THAT WERE
...
;	we can print
	...
	CALL	Z,DECHRT	;IF C=0, THE LAST THING WAS A DECIMAL POINT,
				...
				; "DEC HL" AND NOT THE "MOV	M,B" AT FOFXIS
	POP	AF		;GET THE EXPONENT BACK
	JR	C,FFXLV12	;EXPONENT=0 IF THE NUMBER IS ZERO
...
FOUTNV	PUSH	DE		;SAVE (DE)
	...
	PUSH	AF		;SAVE IT
	FTYPE			;GET TYPE OF NUMBER TO BE PRINTED
...
FOUNS1	FTYPE			;SEE WHAT KIND OF VALUE WE HAVE SO
				; WE CAN SEE IF THE FAC IS BIG ENOUGH
	IF	VERSION < 101	;--->
	  JP	PE,FOUNS1D	;	WE HAVE A DBL
	ELSE			;<-->
	  JR	NC,FOUNS1D	;	WE HAVE A DBL
	ENDIF			;<---
...
FOUNSC	FTYPE	
	...
	LD	DE,23F8H	;Constant = LSW 999999.499
	CALL	FCOMP		
	JR	FOUNSCC
...
FOTTZR	JR	NZ,FOTTZR2	;ENTRY AFTER A "CALL FOUTCV"
FOTTZR1	RET	Z		;RETURN IF WE ARE DONE
	CALL	FOSEDP		;SEE IF WE HAVE TO PUT A COMMA
				; OR A DECIMAL POINT BEFORE THIS ZERO
FOTTZR2	LD	(HL),'0'	;PUT A ZERO IN THE BUFFER
	INC	HL		;UPDATE THE BUFFER POINTER
	DEC	A		;DECREMENT THE ZERO COUNT
	JR	FOTTZR1		;GO BACK AND SEE IF WE ARE DONE

;	Compute B = # digits & C = # commas
;	Here to put a possible comma count in C, and zero C if we are not
;	using the comma specification
FOUSET	LD	A,E		;SETUP DECIMAL POINT COUNT
	ADD	A,D
	INC	A
	LD	B,A
	INC	A		;SETUP COMMA COUNT
FOUSET1	SUB	03H		;REDUCE [A] MOD 3
	JR	NC,FOUSET1
	ADD	A,05H		;ADD 3 BACK IN AND ADD 2 MORE FOR SCALING
	LD	C,A		;SAVE A POSSIBLE COMMA COUNT
FOUSET2	LD	A,(TEMP3)	;GET THE FORMAT SPECS
...
FOSEDP	DEC	B		;TIME FOR D.P.?
	JR	NZ,FOUSEP	;IF NOT D.P. TIME, SEE IF COMMA TIME
...
FOUSEP	DEC	C		;IF ZERO TIME FOR COMMA
...
;	Here to convert a double precision number to decimal digits
	...
	LD	DE,DOUTBL	;GET A POINTER TO THE DBL POWER OF TEN TABLE
...
;	Loop 10 times
FDPL10	CALL	FOSEDP		;SEE IF WE HAVE TO PUT IN A DP OR COMMA
...
FDPDVS	INC	B		;INCREMENT THE DIGIT COUNT
	POP	HL		;GET THE POINTER TO THE POWER OF TEN
	PUSH	HL		;SAVE IT AGAIN
	CALL	DSUBFO		
	JR	NC,FDPDVS	;IF THE NUMBER WAS NOT LESS THAN THE POWER OF TEN,
				; SUBTRACT AGAIN
	...
	JR	NZ,FDPL10	;NO, GO DO THE NEXT ONE
...
FOUCSP	PUSH	BC		;SAVE THE DECIMAL POINT AND COMMA COUNTS
	PUSH	HL		;SAVE THE BUFFER POINTER
	CALL	FADDH		;ROUND NUMBER TO NEAREST INTEGER
	INC	A		;MAKE A NON-ZERO, SINCE NUMBER IS POSITIVE AND
...
FOUCSP1	POP	HL		;GET THE BUFFER POINTER BACK
	POP	BC		;GET THE DECIMAL POINT AND COMMA COUNTS BACK
	XOR	A		;CLEAR CARRY, THE CARRY IS OUR FLAG
				; TO CALCULATE TWO DIGITS
	LD	DE,FOUTBL	;GET POINTER TO POWER OF TEN TABLE
;	Here to calculate the next digit of the number
FOUCSP2	CCF			;COMPLEMENT FLAG THAT TELLS WHEN WE ARE DONE
	CALL	FOSEDP		;SEE IF A COMMA OR DP GOES BEFORE THIS DIGIT
...
FSPDVS	INC	B		;ADD ONE TO DIGIT
	...
	DEC	HL
	JR	NC,FSPDVS	;SUBTRACT AGAIN IF RESULT WAS POSITIVE
	...
	JR	C,FOUCSP2	;CALCULATE NEXT DIGIT IF WE HAVE NOT DONE 2
...
$FOTCI	PUSH	DE		;SAVE (DE)
	LD	DE,IOUTBL	;GET POINTER TO THE INTEGER POWER OF TEN TABLE
...
FCI20	CALL	FOSEDP		;SEE IF A COMMA OR DP GOES BEFORE THE DIGIT
	PUSH	BC		;SAVE COMMA AND DECIMAL POINT INFORMATION
	PUSH	AF		;SAVE DIGIT COUNT
	PUSH	HL		;SAVE BUFFER POINTER
...
FCI30	INC	B		;INCREMENT THE DIGIT COUNT
	...
	CALL	FOSEDP		;YES, SEE IF A DP GOES AFTER THE LAST DIGIT
...
;	1 x 10^15 (DBL)
D1E15	DB	0FDH,0FFH,9FH,31H,0A9H,5FH,63H,0B2H	;999,999,999,999,999.5
...
;	1 x 10^16 (DBL)
D1E16	DB	00H,00H,04H,0BFH,0C9H,1BH,0EH,0B6H	; 1D16: 10000000000000000
;	Powers of 10 (DBL) table [1E15-1E6] int portion only 
DOUTBL	DB	00H,80H,0C6H,0A4H,7EH,8DH,03H		; 1D15: 1000000000000000
...
;	Negate number in the FAC - a.k.a. NEGAFT
...
LOOPER	LD	A,(HL)		;SEE WHAT TYPE OF THING IS ON THE STACK
	...
	EX	DE,HL
	COMPAR	
POPGOF	LD	BC,14		;TO WIPE OUT A FOR ENTRY
	POP	HL
...
LOOP	LD	B,H		;IF EXITING BECAUSE OF END OF PROGRAM,
	...
	LD	L,A
	COMPAR			;SEE IF IT MATCHES OR IF WE'VE GONE TOO FAR
...
;	No DATA token
NODATTK	SUB	REMTK-':'	;WAS IT A REM STATEMENT
...
NOTOL	EX	DE,HL		;[H,L]=TEXT POINTER
	...
	EX	(SP),HL		;SAVE IT ON THE STACK AND REGET THE
				;TEXT POINTER
	LD	DE,1		;DEFAULT THE STEP TO BE 1
...
ONEON	PUSH	BC		;PUT VALUE ON BACKWARDS
	...
	FTYPE			;MUST PUT ON INTEGER/SINGLE-PRECISION FLAG
...
NUMCMD	EQU	SCRATK-ENDTK+1
	CP	NUMCMD		;SOME RESERVED WORD,BUT NOT
...
MORLIN	CHRGET	
	...
	PUSH	HL
	PUSH	AF
	LD	HL,6552		;SEE IF THE LINE # IS TOO BIG (=65520/10)
	COMPAR	
...
GOSUB	LD	C,03H		;MAKE SURE THERE IS ROOM
	...
	LD	A,GOSUTK	;LEAVE A GOSUB TOKEN
				;ON THE STACK
	PUSH	AF
...
INPUTC	LD	BC,NOTQTI	;
	...
	SYNCHK	';'		;MUST END WITH SEMI-COLON
...
LOPDT2	SYNCHK	','		;MAKE SURE THERE IS A ","
...
DATBK	CALL	L3INVAR		;L3: Called before assigning data to variable
	FTYPE			;SEE IF ITS NUMERIC OR STRING
				;IS IT A STRING ?
...
FRMEVLP	SYNCHK	'('		;GET PAREN BEFORE FORMULA
...
FINREL	LD	A,B		;[A]=OLD PRECEDENCE
	...
	PUSH	HL
	FTYPE			;SEE IF WE HAVE A NUMERIC COMPARE
...
NTVARP	CP	USRTK
	...
	JP	Z,FNDOER	
	SUB	SGNTK		;IS IT A FUNCTION NAME?
...
RETVAR	PUSH	HL		;SAVE THE TEXT POINTER
	...
	FTYPE			;FOR STRINGS WE JUST LEAVE
...
LOPFND	EX	DE,HL		;[D,E]=POINTER INTO SIMPLE VARIABLES
	...
	COMPAR			;SEE IF THE END HAS BEEN REACHED
	POP	HL		;[H]=VALTYP
...
NOTFNS	LD	A,H
	...
	COMPAR			;IF SO, DON'T MAKE A NEW VARIABLE
...
ZEROER	DEC	HL		;[H,L] IS RETURNED POINTING TO THE
	...
	COMPAR			;ZERO BACWARDS TO [D,E] WHICH
...
FRE	LD	HL,(STREND)	;GET END OF VARIABLE AND TEXT SPACE
	...
	FTYPE			;WAS THE ARGUMENT A STRING?
...
;	STRING$()
STRNGER	CHRGET			;GET NEXT CHAR FOLLOWING "STRING$"
	...
	FTYPE			;GET TYPE OF ARG
	JR	Z,STRNGR1	;WAS A STRING
	CALL	CONINT		;GET ASCII VALUE OF CHAR
	JR	STRNGR2		;NOW CALL SPACE CODE

STRNGR1	CALL	ASC1		;GET VALUE OF CHAR IN [A]
STRNGR2	POP	DE		;GET REP FACTOR IN [E]
	PUSH	AF		;SAVE CHAR
	PUSH	AF		;SAVE CHAR
	LD	A,E		;GET NUMBER OF CHARS IN [A]
	CALL	STRINI		;GET A STRING THAT LONG
	LD	E,A		;COUNT OF CHARS BACK IN [B]
	POP	AF		;GET BACK CHAR TO PUT IN STRING
	INC	E		;TEST FOR NULL STRING
	DEC	E
	JR	Z,FINBCK	;YES, ALL DONE
	LD	HL,(DSCTMP2)	;GET DESC. POINTER
STRNGR3	LD	(HL),A		;SAVE CHAR
	INC	HL		;BUMP PTR
	DEC	E		;DECR COUNT
	JR	NZ,STRNGR3	;KEEP STORING CHAR
...
MID$	EX	DE,HL		;PUT THE TEXT POINTER IN [H,L]
	...
	SYNCHK	','		;COMMA? MUST DELINEATE 3RD ARG.
...
PLOOP1	LD	A,(HL)		;GET A CHARACTER FROM LINE.
	...
	INC	D
;	No "'" token
XNOSQTK	CP	ELSETK		;IF ITS "ELSE" DON'T PRINT THE COLON
				;IN FRONT OF IT
...
NEWUCH	CALL	PLSPRT		;IF A "+" CAME BEFORE THIS CHARACTER
	...
	CALL	OUTCHR		;PRINT THE CHARACTER THAT WASN'T
				;PART OF A FIELD
...
PLSFIN	CALL	PLSPRT		;ALLOW FOR MULTIPLE PLUSES
	...
	JR	NZ,NEWUCH	;IF NOT, ITS NOT PART
				...
				;HAVE BEEN TRIED
	LD	A,B		;CHECK FOR $
	CP	02H		;SEE IF THE "USING" STRING IS LONG
	INC	HL		;ENOUGH FOR THE SPECIAL CASE OF
	JR	C,NOTSPC	; **$
	LD	A,(HL)
...
;	be scanned
	...
	LD	C,(HL)		;THE "USING" STRING'S STRING DATA
	INC	HL		;SETUP [D,E] AS A DOUBLE BYTE OFFSET
	LD	H,(HL)
	LD	L,C		;THE "USING" STRING'S STRING DATA
	LD	D,00H		;SETUP [D,E] AS A DOUBLE BYTE OFFSET
	LD	E,A		;THE "USING" STRING'S STRING DATA
...
REUSIN	CALL	PLSPRT		;PRINT A "+" IF NECESSARY
	CALL	OUTCHR		;PRINT THE FINAL CHARACTER
...
FLDFIN	CALL	C,CRDO1		;IF NOT COMMA OR SEMI-COLON
...
PLSPRT	PUSH	AF		;SAVE THE CURRENT CHARACTER
	...
	CALL	NZ,OUTCHR	;PRINT IT IF THE BIT WAS SET
