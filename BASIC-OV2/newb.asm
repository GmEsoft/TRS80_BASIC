;NEWB/ASM
*LIST OFF
	TITLE	'<NEWB - New additions to BASIC>'
;
;	CAUTION - labels contain @ or $ may be used by the
;	BASIC/OV2 overlay.  If changes are made to this module,
;	it must be xrefed and the /OV2 modules re-assembled.
;
;
CR	EQU	0DH
DTOKE	EQU	0AAH
CURLIN	EQU	7192H		;Current line number
OLDHI	EQU	7151H		;HIGH$ upon entry to BASIC
OLDINIT	EQU	8048H		;End of BASIC before this
DOSCLIN	EQU	8044H		;Ptr to DOS cmdline on entry
BASEND	EQU	6E9AH		;Eventually, top of free mem
MAXSPAC	EQU	6E9CH
STARTUP	EQU	8028H		;Execute BASIC after init
INBUF	EQU	7039H		;Input buffer
PGMBGN	EQU	6E9EH		;Ptr to program start
RENUM	EQU	658EH		;Entry pt for renum
RENUM1	EQU	65B5H		;Renum intercept for ending line
RENUM2	EQU	6604H		;Intercept for endline check
SYNERR	EQU	586CH		;Print "Syntax error
FUNCERR	EQU	5F95H		;Print "Illegal function call
DERR	EQU	5282H		;Print error returned
OMERR	EQU	4CD9H		;Out of mem, pgm load only!
;
IMBACK	EQU	7E2AH		;Back to basic's keyin
FIND	EQU	5AB4H		;Find a line
CPHLDE	EQU	434DH		;Compare HL and DE
DOEDIT	EQU	3D27H		;Edit a line
TOBUFF	EQU	5AD3H		;Conv toupper and move
NXTCHAR	EQU	5E93H		;Get next char fm line buff
NUM2DE	EQU	5F9AH		;Get a number to DE
UNTOKE	EQU	7A0DH		;Expand a tokenized line
FILEX	EQU	6E98H		;DCB-2
HIPT	EQU	7178H		;Hi ptr used during load
CLEAR	EQU	447EH		;Entry to CLEAR statement
READY	EQU	5920H		;Print Ready msg
NORMNUM	EQU	661EH
OV1	EQU	7F1FH		;Re-load /OV1
;
DOSOVR@	EQU	01E00H
OVRLY$	EQU	69H
;
	ORG	5990H		;Basic line in intercept
	CALL	IMMED
;
	ORG	59BFH		;Line parse
	CALL	ABBR
;
	ORG	4C91H		;Program load
	CALL	FASTLD
	JP	4C98H		;Continue check
;
	ORG	4D76H
...
SVCUSR:	CALL	NXTCHAR		;Get a char
	...
	LD	A,(7149H)	;Get the number
	CP	11		;Is it ours?
	JR	Z,SVC1		;Ok if so
	POP	AF		;If not, return
	JP	773AH		;Back to basic
...
SFLAG1	EQU	$-2
	BIT	5,A		;JCL?
	JP	NZ,7E1BH	;Don't do immediate if JCL
	LD	HL,INBUF	;Keyboard input buffer
	CALL	45DFH		;Chars left check??
	JP	NZ,7E42H	;Go if still on last line
...
DOLINE:	LD	E,(HL)
	...
	CALL	57E8H		;Don't know what this does
...
LISTLIN@
	...
	CALL	3139H		;Display the line #
	POP	HL
	LD	A,(HL)
	CP	9
	LD	A,' '
	CALL	NZ,40D8H	;Display space after line
LSTCAL:	CALL	UNTOKE		;Make line into ascii
	LD	HL,INBUF	;Pt to start of line
	CALL	7A04H		;Display line
	JP	41F5H		;Dsply CR and return
;
;	Do abbreviated keys
;
ABBR:
...
RENB:
	...
	CALL	4353H		;Ck it out
	DB	2CH		;Valid separator
	CALL	5FA4H		;Shouldn't be any more
	JP	NZ,SYNERR
	CALL	FIND
...
BEGINA	INC	A
	...
	LD	(BASEND),HL
	CALL	7F1FH		;Load /OV1
	CALL	4317H		;Init ptrs, set stack
	LD	A,':'
	LD	(6EF9H),A
	LD	HL,0FFFEH
	LD	(MAXSPAC),HL
