...
NTSTOP?	LD	HL,(AUTINC)	;GET INCREMENT
	...
	JP	C,NTAUTO?	;CHECK FOR PATHETIC CASE - TODO label
	PUSH	DE		;SAVE LINE NUMBER #
	LD	DE,0FFF9H	;CHECK FOR LINE # TOO BIG
	CALL	COMPAR
	POP	DE		;GET BACK LINE #
	JP	NC,NTAUTO?	;IF TOO BIG, QUIT
...
;	No ELSE (skip deleting preceding ':')
	...
	JP	NZ,BFNTWH	;SO SKIP OVER IT IF ITS THERE
...
;	Next initial letter
RESR3	INC	C		;BUMP LEADING CHAR VALUE.
;	Next word with same initial
RESR1	INC	HL		;BUMP POINTER INTO RESLST
	LD	D,H		;SAVE PTR TO START OF THIS RESWRD
	LD	E,L
;	Next letter
RESR2	LD	A,(HL)		;GET CHAR FROM RESLST
	OR	A		;SET CC'S
	JP	Z,RESR3		;IF END OF THIS CHARS TABLE, GO BACK & BUMP C
	INC	HL		;BUMP SOURCE PTR
	JP	P,RESR2		;IF NOT END OF THIS RESWRD, THEN KEEP LOOKING
	LD	A,(HL)		;GET PTR TO RESERVED WORD VALUE
	CP	B		;SAME AS THE ONE WE SEARCH FOR?
	JP	NZ,RESR1	;NO, KEEP LOOKING.
...
CONSD	CALL	$MOVRF		;GET THE HO'S IN THE REGISTERS
	CALL	SETSNG		;SET VALTYP TO "SINGLE PRECISON"
...
CHKSTR	CALL	GETYPR		 ;SEE WHAT KIND OF VALUE WE HAVE
	...
	JP	TMERR		 ;WE DON'T HAVE A STRING, FALL INTO TMERR;Type Mismatch
...
DINTFO	PUSH	AF		;ENTRY FROM FOUT, CARRY IS ZERO
	...
	LD	(DFACLO1),A	; WE NORMALIZE, WE WILL SHIFT IN ZEROS;DFACLO1
...
DMULT3	PUSH	BC		;SAVE COUNTERS
	...
	LD	B,A		;SAVE IT
	CALL	C,DADDAR	;ADD IN OLD FAC IF BIT OF MULTIPIER WAS ONE
...
FUNMLT	PUSH	AF		;SAVE EXPONENT, ENTRY FROM FOUT
...
;	JR	NZ,OMERR	;Can't garbage collect if CHAINing
	...
	RET	NC		;YES
	JP	OMERRS		;NO, GIVE "OUT OF MEMORY BUT DONT TOUCH STACK
...
;
;
SYSTEM	RET	NZ		;SHOULD TERMINATE
